...
# Global_performance_with_ite_cuda_sz3_NYX_rank_0_16.txt
txt_files = sorted(
    glob.glob("Global_performance_with_ite_cuda_sz3_NYX_rank_0_*_1.txt"),
    key=lambda x: int(re.search(r"rank_0_(\d+)_1\.txt", x).group(1))
)
...
for fname in txt_files:
    match = re.search(r"rank_0_(\d+)_1\.txt", fname)
    if not match:
        continue
...
# 标题和样式
# plt.title("Weak Scaling: Comp Time and Efficiency")
# --- Legend ---
...
plt.tight_layout()
plt.savefig("weake_scaling_pMSz.svg")
plt.show()
