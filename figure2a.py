...
txt_files = sorted(
    glob.glob("Global_naive_MSz_performance_with_ite_cuda_sz3_NYX_rank_0_*.txt"),
    key=lambda x: int(re.search(r"rank_0_(\d+)\.txt", x).group(1))
)
...
for fname in txt_files:
    match = re.search(r'rank_0_(\d+)', fname)
    if not match:
        continue
...
plt.tight_layout()
plt.savefig("weake_scaling_naive_pMSz.svg")
plt.show()
