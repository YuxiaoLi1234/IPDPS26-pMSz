...
txt_files = sorted(
    glob.glob("path/to/your/strong_scale/Global_performance_with_ite_cuda_sz3_perlin_1024_1024_1024_rank*_.txt"),
    key=lambda x: int(re.search(r"rank(\d+)_\.txt", x).group(1))
)
...
for fname in txt_files:
    match = re.search(r'rank(\d+)_', fname)
    if not match:
        continue
...
efficiency = [avg_comps[0] / (t * num_ranks[i]) for i,t in enumerate(avg_comps)]  # 相对于第一个rank的效率
...
# 标题和样式
# plt.title("Strong Scaling: Comp Time and Efficiency")
