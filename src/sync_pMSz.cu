...
int main(int argc, char** argv) {
    ...
    PY = std::stoi(argv[5]);
    PZ = std::stoi(argv[6]); 
    std::string result_dir = (argc > 7) ? argv[7] : "";
    check = 0; 
    ...
    if(rank == 0 && compression == 1 && compression_mode == 0){
        ...
        if(compressor_id == "sz3") {
            ...
            auto start = std::chrono::high_resolution_clock::now();
            cpfilename = "../datasets/compressed_"+filename+"_"+std::to_string(bound)+".zfp";
    ...
    MPI_Barrier(cart_comm);
    int c1 = 0;  
    std::ofstream outFilep("../gannt_results/sync_pMSz/"+filename+"_"+compressor_id+"_rank_"+std::to_string(rank)+".txt", std::ios::app);
    ...
    if(check_consistency == 1 && rank == 0){
        ...
        std::ofstream fout("../datasets/fixed_" + filename + std::to_string(bound) + "_" + compressor_id+ ".bin", std::ios::out | std::ios::binary);
        if (!fout) {
    ...
    if(rank == 0){
        ...
        std::cout<<"Edit ratio is: "<< edit_ratio << " total_edited:" << total_edited<< std::endl;
        
        std::ofstream outFile3("../stat_result/"+result_dir+"/sync_pMSz/"+filename+"_"+compressor_id+"_"+std::to_string(size)+".txt", std::ios::app);
        ...
        outFile3 << std::setprecision(10)<< "Overall CR: "<< OCR << std::endl;
        outFile3 <<std::setprecision(10)<< "Original CR: "<< CR << std::endl;
        ...
        outFile3 << std::setprecision(17)<<"Correction_time: "<< additional_time<< std::endl;
