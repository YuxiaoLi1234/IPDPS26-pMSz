...
txt_files = sorted(
    glob.glob("../gannt_results/sync_pMSz/perlin_*_*_*_sz3_rank_*.txt"),
    key=lambda x: int(re.search(r"_rank_(\d+)\.txt", x).group(1))
)
print(txt_files)
all_comps = []
all_comms = []
...
for fname in txt_files:
    match = re.search(r"_rank_(\d+)\.txt", fname)
    if not match:
        continue
...
all_comps  = [ [v for v in x[1]] for x in sorted_data ] 
all_comms  = [ [v for v in x[2]] for x in sorted_data ]

avg_comps =  [np.mean(all_comps[i]) for i, comp in enumerate(all_comps)]
avg_comms =  [np.mean(comm) for i, comm in enumerate(all_comms)]
efficiency = [avg_comps[0] / (t + avg_comms[i]) for i, t in enumerate(avg_comps)] 
...
ax1.set_ylim(0, max(avg_comps + avg_comms) + 0.01)
ax1.tick_params(axis='y', labelcolor='black', labelsize=14)


for i in range(len(avg_comps)):
    ax1.text(x[i], avg_comps[i] + avg_comms[i] + 0.001, f"{avg_comps[i]:.4f}", ha='center', fontsize=14)
...
plt.savefig("../figures/figure10d.svg", dpi=300)
plt.show()
